...
#-------------------------------------------------------------------------------
import os, sys, time, subprocess, urllib, urllib.request
import simplejson
import xpath
    ...
    for vd in video_details:
        ...
        if elems:
            ...
            try:
                ...
                dynamic_video = json['data']['initialVideo']['videoPlayInfo']['dynamic_video']
            ...
            if dynamic_video_url:
                video_path = os.path.join(video_temp_dir, title+'_video.mp4')
                print('\ndynamic_video_url=%s' %dynamic_video_url)
                _download(dynamic_video_url, video_path)
            ...
            if dynamic_audio_list:
                ...
                audio_path = os.path.join(video_temp_dir, title+'_audio.m4a')
                print('\ndynamic_audio_url=%s' %dynamic_audio_url)
                _download(dynamic_audio_url, audio_path)
            # merge video / audio files
            if video_path and audio_path:
                final_video_path = os.path.join(video_output_dir, title+'.mp4')
                _merge_video(video_path, audio_path, final_video_path)
                print('\nSuccessfully output video: %s' %final_video_path)
                time.sleep(5)
            #print(json_str)
        #sys.exit(0)
